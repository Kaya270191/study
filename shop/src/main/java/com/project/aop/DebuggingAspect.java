...
public class DebuggingAspect {
    ...
    //  대상 메서드 선택: api 패키지의 모든 메소드
    @Pointcut("execution(* com.project.api.*.*(..))")
    private void cut(){}

    //실행 시점 설정: cut()의 대상이 수행되기 이전
    @Before("cut()")
    public void loggingArgs(JoinPoint joinPoint){ //cyt()의 대상 메소드
        //입력값 가져오기
    ...
    public void loggingReturnValue(JoinPoint joinPoint, //cut()의 대상 메소드
        ...
        log.info("{}#{}의 반환값값 => {}", className, methodName, returnObj);
